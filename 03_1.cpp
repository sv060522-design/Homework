...
int main() {
    sf::RenderWindow window(sf::VideoMode(800, 800), "Raw Pointers");
    window.setFramerateLimit(60);
    sf::Font font;
    if (!font.loadFromFile("sourceCodePro.ttf")) return 1;

    std::vector<Widget*> widgets;
    widgets.push_back(new Button(window, {50, 50, 150, 50}, font, "Click Me"));
    widgets.push_back(new Slider(window, {400, 400}, {300, 10}, {20, 40}));
    widgets.push_back(new Draggable(window, {100, 200}, {100, 100}, sf::Color::Red));

    while (window.isOpen()) {
        sf::Event event;
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed) window.close();
            for (Widget* w : widgets) w->handleEvent(event);
        }

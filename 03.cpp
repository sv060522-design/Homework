...
uint64_t getMax(int n, const std::vector<uint64_t>& v)
{
    ...
    for (size_t i = 0; i < k; ++i)
    {
        ...
        std::packaged_task<uint64_t()> task([&v, l, r]()
        {
            uint64_t res = v[l];
        ...
        f.push_back(task.get_future());
        t.push_back(std::thread(std::move(task)));
    }
...
int main()
{
    ...
    cout << "Packaged task maximum = " << m2 << endl;
    cout << "Packaged task time = "
